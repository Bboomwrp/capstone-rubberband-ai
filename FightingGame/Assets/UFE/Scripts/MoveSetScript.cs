...
public class BasicMoveInfo:ICloneable {
    ...
    [HideInInspector] public bool bpVisibleToggle;
...
public class MoveSetScript : MonoBehaviour {
			...
			if (moveSetData.combatStance == newStance){
                ...
                if (currentClip != null) {
                    ...
                    if (controlsScript.myInfo.animationType == AnimationType.Mecanim) {
                        ...
                        mecanimControl.animator.Play(currentState, 0, currentNormalizedTime);
        ...
        if (hitBoxes != null)
        {
            foreach (HitBox hitBox in hitBoxes.hitBoxes)
            {
                if (hitBox != null && hitBox.bodyPart != BodyPart.none && hitBox.position != null)
                {
                    ...
                    foreach (BodyPartVisibilityChange visibilityChange in basicMove.bodyPartVisibilityChanges)
                    {
                        if (visibilityChange.bodyPart == hitBox.bodyPart &&
                            ((controlsScript.mirror == -1 && visibilityChange.left) || (controlsScript.mirror == 1 && visibilityChange.right)))
                        {

                            UFE.FireBodyVisibilityChange(null, controlsScript.myInfo, visibilityChange, hitBox);
    ...
    // Check Ultra gauge
    private bool hasEnoughUltraGauge(float gaugeNeeded)
    {
        if (!UFE.config.gameGUI.hasUltraGauge) return true;
        if (controlsScript.myInfo.currentUltraGaugePoints < (controlsScript.myInfo.maxUltraGauge * (gaugeNeeded / 100))) return false;
        return true;
    }
	...
	public MoveInfo GetMove(ButtonPress[] buttonPress, float charge, MoveInfo currentMove, bool inputUp, bool forceExecution){
        ...
        if (controlsScript.currentMove != null && controlsScript.currentMove.isUltraMove) return null;
    ...
    public bool ValidateMoveExecution(MoveInfo move) {
        ...
        if (!hasEnoughUltraGauge(move.useUltraGauge ? 50 : 0)) return false;
	...
	public bool ValidateMoveStances(PlayerConditions conditions, ControlsScript cScript, bool bypassCrouchStance){
		...
		foreach(PossibleMoveStates possibleMoveState in conditions.possibleMoveStates){
            ...
            if (possibleMoveState.isCornor)
            {
                if (!(cScript.normalizedDistance == 1 ||
                    cScript.character.transform.position.x == UFE.config.selectedStage.leftBoundary ||
                    cScript.character.transform.position.x == UFE.config.selectedStage.rightBoundary))
                    continue;
            }
            else
            {
	...
	private MoveInfo TestMoveExecution(MoveInfo move, MoveInfo currentMove, ButtonPress[] buttonPress, bool inputUp, bool fromSequence, bool forceExecution) {
        ...
        if (!hasEnoughUltraGauge(move.useUltraGauge ? 50 : 0)) return null;
