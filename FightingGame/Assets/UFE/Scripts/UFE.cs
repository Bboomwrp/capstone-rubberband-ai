...
public class UFE : MonoBehaviour {
	...
	private static InputTouchControllerBridge touchControllerBridge;
	...
	protected void Awake(){
        ...
        if(UFE.eventSystem.gameObject == go) {
        ...
        watermark.transform.SetParent(go.transform);

        RectTransform trans = watermark.AddComponent<RectTransform>();
        trans.anchoredPosition = new Vector2(0, -(Screen.height));
        //Screen.height / 720

        Text watermarkText = watermark.AddComponent<Text>();
        watermarkText.text = "Trial Version";
        watermarkText.alignment = TextAnchor.LowerCenter;
        watermarkText.color = Color.white;

        Font ArialFont = (Font)Resources.GetBuiltinResource(typeof(Font), "Arial.ttf");
        watermarkText.font = ArialFont;
        watermarkText.fontSize = 40;
        watermarkText.verticalOverflow = VerticalWrapMode.Overflow;
        watermarkText.horizontalOverflow = HorizontalWrapMode.Overflow;
        watermarkText.material = ArialFont.material;

        Outline watermarkOutline = watermark.AddComponent<Outline>();
#endif
    }
...
#region private class methods: GUI Related methods
	...
	private static void _StartGame(float fadeTime){
        ...
        UFE.config.player1Character.currentUltraGaugePoints = 0;
        UFE.config.player2Character.currentUltraGaugePoints = 0;
        UFE.config.player1Character.currentStunPoints = 0;
        UFE.config.player2Character.currentStunPoints = 0;
