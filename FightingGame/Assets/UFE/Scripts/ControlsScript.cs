...
public class ControlsScript : MonoBehaviour {
    ...
    public float currentStunDecreaseTime;
    ...
    private bool isFullPowerUltra;

	public float storedMoveTime;

	public float attackMultiplier = 1f;
	public float defenseMultiplier = 1f;
	public float gaugeGainMultiplier = 1f;
...
*/
    void Start() {
		...
		if (gameObject.name == "Player1") {
            ...
            if (UFE.gameMode == GameMode.TrainingRoom) {
                ...
                myInfo.currentUltraGaugePoints = (float)myInfo.maxUltraGauge * (UFE.config.trainingModeOptions.p1StartingUltraGauge / 100);
            }
            else {
                myInfo.currentLifePoints = (float)myInfo.lifePoints;
            }
            myInfo.currentStunPoints = 0;
        }
        else{
            ...
            if (UFE.gameMode == GameMode.TrainingRoom) {
                ...
                myInfo.currentUltraGaugePoints = (float)myInfo.maxUltraGauge * (UFE.config.trainingModeOptions.p2StartingUltraGauge / 100);
            }
            else {
                myInfo.currentLifePoints = (float)myInfo.lifePoints;
            }
	...
	public void DoFixedUpdate(){
        ...
        if ((playerNum == 1 && UFE.gameMode == GameMode.TrainingRoom && UFE.config.trainingModeOptions.p1UltraGauge == LifeBarTrainingMode.Refill) ||
            (playerNum == 2 && UFE.gameMode == GameMode.TrainingRoom && UFE.config.trainingModeOptions.p2UltraGauge == LifeBarTrainingMode.Refill))
        {
            if (!UFE.FindDelaySynchronizedAction(this.RefillUltraGauge))
                UFE.DelaySynchronizedAction(this.RefillUltraGauge, UFE.config.trainingModeOptions.refillTime);
        }
        ...
        if (UFE.gameMode == GameMode.TrainingRoom && myInfo.currentUltraGaugePoints < myInfo.maxUltraGauge &&
            ((playerNum == 1 && UFE.config.trainingModeOptions.p1UltraGauge == LifeBarTrainingMode.Infinite) ||
            (playerNum == 2 && UFE.config.trainingModeOptions.p2UltraGauge == LifeBarTrainingMode.Infinite))) RefillUltraGauge();
        ...
        // Play stun move
        if (isStunned && currentSubState != SubStates.Stunned && currentState == PossibleStates.Stand)
        {
            CastMove(myInfo.moves[0].stunnedMove[0], true,true);
//            myMoveSetScript.animationsadf(myMoveSetScript.basicMoves.stunned, myMoveSetScript.basicMoves.stunned.name, 0f, false, true);
            isStunned = false;
        }

        // Stun Decrease
        if (myInfo.currentStunPoints >= myInfo.maxStunPoints && myInfo.currentLifePoints > 0)
        {
            ...
            myMoveSetScript.PlayBasicMove(myMoveSetScript.basicMoves.getHitSweep, myMoveSetScript.basicMoves.getHitSweep.name, 0f, false, true);
            currentSubState = SubStates.Stunned;
            stunTime = UFE.config.stunStandupTime;
            currentState = PossibleStates.Down;
            //            myPhysicsScript.ForceGrounded();
            Debug.Log("sweep name : " + myMoveSetScript.basicMoves.getHitSweep.name);
            isStunned = true;
        }
        else if (myInfo.currentStunPoints > 0)
        {
            if (currentStunDecreaseTime > 0)
            {
                //                Debug.Log("current stun decrease time : " + currentStunDecreaseTime);
                currentStunDecreaseTime -= Time.fixedDeltaTime;
            }
            else
            {
                myInfo.currentStunPoints -= Time.fixedDeltaTime * UFE.config.stunDecreaseSpeed;
            }
        }
...
//        if (currentMove != null && currentMove.isUltraMove) return;

//        print("input ref : " + inputController.inputReferences.Count);
		...
		foreach (InputReferences inputRef in inputController.inputReferences) {
			...
			if (inputRef.inputType != InputType.Button && inputHeldDown[inputRef.engineRelatedButton] > 0 && inputController.GetAxisRaw(inputRef) == 0) {
                ...
                if (myInfo.physics.pressureSensitiveJump
                    ...
                    float jumpDelaySeconds = (float)myInfo.physics.jumpDelay / UFE.config.fps;
                    float pressurePercentage = Mathf.Min(inputHeldDown[inputRef.engineRelatedButton] / jumpDelaySeconds, 1f);
                    float newJumpForce = Mathf.Max((myInfo.physics.jumpForce * pressurePercentage), myInfo.physics.minJumpForce);
	...
	public void ReadMove(MoveInfo move){
		...
		if (move.currentTick == 0) {
            ...
            if (move.useUltraGauge)
            {
                RemoveUltraGauge();
            }
            if (move.isUltraMove && !isFullPowerUltra)
            {
                foreach (var hit in move.hits)
                {
                    hit.damageOnHit = hit.damageOnHit * 0.67f;
                    hit.damageOnBlock = hit.damageOnBlock * 0.67f;
                }
            }
		...
		foreach (Projectile projectile in move.projectiles){
			if (
                ...
                projectile.isUltra = move.isUltraMove;
                if (projectile.isUltra && !isFullPowerUltra)
                {
                    projectile.damageOnHitFirst = projectile.damageOnHitFirst * 0.67f;
                    projectile.damageOnBlockFirst = projectile.damageOnBlockFirst * 0.67f;
                    projectile.damageOnHit = projectile.damageOnHit * 0.67f;
                    projectile.damageOnBlock = projectile.damageOnBlock * 0.67f;
                    projectile.damageOnHitLast = projectile.damageOnHitLast * 0.67f;
                    projectile.damageOnBlockLast = projectile.damageOnBlockLast * 0.67f;
                }
            ...
            if (!visibilityChange.casted && move.currentFrame >= visibilityChange.castingFrame) {
                foreach (HitBox hitBox in myHitBoxesScript.hitBoxes) {
                    if (visibilityChange.bodyPart == hitBox.bodyPart &&
                        ...
                        if (visibilityChange.saveAsDefault) hitBox.defaultVisibility = visibilityChange.visible;
                    }
                }
            }
        }
...
//                Debug.Log("print 0");
                ...
                opControlsScript.activePullIn.forceStand = opponentOverride.forceGround;
...
//                opControlsScript.transform.position = transform.position + newPos;
//                print(transform.gameObject.name + " __ " + transform.position + " __ " + newPos + " opp " + opControlsScript.gameObject.name + " position " + opControlsScript.transform.position);
				...
				if (hit.hurtBoxes.Length > 0){
					...
					if (collisionVectors.Length > 0) { // HURTBOX TEST
						...
						// Block
							...
							opControlsScript.GetHitBlocking(hit, move.totalFrames - move.currentFrame, collisionVectors, move.isUltraMove);
						...
						// Hit
							...
							opControlsScript.GetHit(hit, move.totalFrames - move.currentFrame, collisionVectors, move.isUltraMove);
//                            Debug.Log("in read move : dmg : " + hit.damageOnHit + " stun : " + hit.stundamage);
                        ...
                        if (!opPhysicsScript.IsGrounded() && hit.applyDifferentAirForce) {
                            myPhysicsScript.AddForce(hit.appliedForceAir, -mirror);
                        } else {
        ...
        // Move to Next Frame
        move.currentFrame = (int)Mathf.Round(move.currentTick);
        

        // Kill move when it ends
	...
	public void GetHitParry(Hit hit, int remainingFrames, Vector3[] location){
        ...
        } else if (hit.spaceBetweenHits == Sizes.Custom) {
            spaceBetweenHits = hit.spaceBetweenHitsVal;
		}
	...
	public void GetHitBlocking(Hit hit, int remainingFrames, Vector3[] location, bool isUltraMove){
		// Lose life
		if (hit.damageOnBlock >= myInfo.currentLifePoints){
			GetHit(hit, remainingFrames, location, isUltraMove);
			return;
		}else{
            // Add Ultra gauge
            AddUltraGauge(hit.damageOnBlock);

//            Debug.Log("block damage : " + hit.damageOnBlock);

            DamageMe(hit.damageOnBlock);
            currentStunDecreaseTime = UFE.config.stunDelayStartTime;
        }
	...
	public void GetHit(Hit hit, int remainingFrames, Vector3[] location, bool isUltraMove){
        ...
        myPhysicsScript.overrideStunAnimation = null;
        if (hit.overrideHitAnimation && (!hit.overrideOnlyOnAirHit || airHit)) {
		...
		if (location.Length > 0 && hitEffects.hitParticle != null){
            ...
            pTemp.transform.eulerAngles = transform.GetChild(0).transform.eulerAngles.y == 90 ?  new Vector3(0f, 180f, 0f) : Vector3.zero;
            pTemp.transform.parent = UFE.gameEngine.transform;
        ...
        } else if (UFE.config.comboOptions.damageDeterioration == Sizes.Custom) {
            damage = hit.damageOnHit - (hit.damageOnHit * (float)comboHits * UFE.config.comboOptions.damageDeteriorationVal);
		}
...
//        Debug.Log("hit damage : " + damage);
        ...
        // Add Ultra gauge
        AddUltraGauge(damage);

        // Add Stun
        AddStunGauge(hit.stundamage);
//        Debug.Log("dmg : " + hit.damageOnHit + " stun dmg : " + hit.stundamage);
        currentStunDecreaseTime = UFE.config.stunDelayStartTime;
        ...
        if ((currentMove == null || !currentMove.hitAnimationOverride) && (!armored || isDead)) {
            ...
            if (!hit.resetPreviousHitStun) {
                ...
                } else if (UFE.config.comboOptions.hitStunDeterioration == Sizes.Custom) {
                    stunTime -= (float)comboHits * UFE.config.comboOptions.hitStunDeteriorationVal;
                }
            }
            ...
            if (pushForce.y > 0 || (isDead && !isKnockDown)) {
                ...
                } else if (UFE.config.comboOptions.airJuggleDeterioration == Sizes.Custom) {
                    pushForce.y -= (pushForce.y * (float)airJuggleHits * UFE.config.comboOptions.airJuggleDeteriorationVal);
                }
            ...
            if ((airHit || (!myPhysicsScript.IsGrounded() && airRecoveryType == AirRecoveryType.DontRecover))
                ...
                if (hit.hitType == HitType.HighKnockdown) {
                    ...
                    myPhysicsScript.overrideAirAnimation = UFE.config.knockDownOptions.high.overrideAirAnimation;
                    ...
                    myPhysicsScript.overrideAirAnimation = UFE.config.knockDownOptions.highLow.overrideAirAnimation;
                    myPhysicsScript.overrideAirAnimation = true;
                    airRecoveryType = AirRecoveryType.DontRecover;
                    if (!hit.customStunValues) stunTime =
                    ...
                    myPhysicsScript.overrideAirAnimation = UFE.config.knockDownOptions.sweep.overrideAirAnimation;
                    myPhysicsScript.overrideAirAnimation = true;
                    airRecoveryType = AirRecoveryType.DontRecover;
                    if (!hit.customStunValues) stunTime = 
    ...
    public void AddGauge(float gaugeGain) {
		...
		float gain = (myInfo.maxGaugePoints * (gaugeGain / 100)) * gaugeGainMultiplier;

        myInfo.currentGaugePoints += gain;
    ...
    // Add Ultra gauge
    public void AddUltraGauge(float gaugeGain)
    {
//        Debug.Log("add ultra gauge");
        if ((isDead || opControlsScript.isDead) && UFE.config.roundOptions.inhibitGaugeGain) return;
        if (!UFE.config.gameGUI.hasUltraGauge) return;
        if (inhibitGainWhileDraining) return;
        myInfo.currentUltraGaugePoints += gaugeGain;
        if (myInfo.currentUltraGaugePoints > myInfo.maxUltraGauge) myInfo.currentUltraGaugePoints = myInfo.maxUltraGauge;
//        Debug.Log("current : " + myInfo.currentUltraGaugePoints);
    }

    // Remove ultra gauge
    private void RemoveUltraGauge()
    {
        if ((isDead || opControlsScript.isDead) && UFE.config.roundOptions.inhibitGaugeGain) return;
        if (!UFE.config.gameGUI.hasUltraGauge) return;
        if (UFE.gameMode == GameMode.TrainingRoom && playerNum == 1 && UFE.config.trainingModeOptions.p1UltraGauge == LifeBarTrainingMode.Infinite) return;
        if (UFE.gameMode == GameMode.TrainingRoom && playerNum == 2 && UFE.config.trainingModeOptions.p2UltraGauge == LifeBarTrainingMode.Infinite) return;

        if (myInfo.currentUltraGaugePoints >= myInfo.maxUltraGauge) isFullPowerUltra = true;
        else isFullPowerUltra = false;

//        float gaugeLoss = myInfo.currentUltraGaugePoints >= myInfo.maxUltraGauge ? 100 : 50;
//        Debug.Log("remove ultra gauge : " + gaugeLoss);
//        myInfo.currentUltraGaugePoints -= (myInfo.maxUltraGauge * (gaugeLoss / 100));
        myInfo.currentUltraGaugePoints = 0f;
//        if (myInfo.currentUltraGaugePoints < 0) myInfo.currentUltraGaugePoints = 0;

        if ((playerNum == 1 && UFE.gameMode == GameMode.TrainingRoom && UFE.config.trainingModeOptions.p1UltraGauge == LifeBarTrainingMode.Refill) ||
            (playerNum == 2 && UFE.gameMode == GameMode.TrainingRoom && UFE.config.trainingModeOptions.p2UltraGauge == LifeBarTrainingMode.Refill))
        {
            if (!UFE.FindAndUpdateDelaySynchronizedAction(this.RefillUltraGauge, UFE.config.trainingModeOptions.refillTime))
                UFE.DelaySynchronizedAction(this.RefillUltraGauge, UFE.config.trainingModeOptions.refillTime);
        }
    }

    // Add Stun Gauge
    public void AddStunGauge(float gaugeGain)
    {
        if (isDead || opControlsScript.isDead) return;
        if (UFE.gameMode == GameMode.TrainingRoom && playerNum == 1 && UFE.config.trainingModeOptions.p1StunGauge != LifeBarTrainingMode.Normal) return;
        if (UFE.gameMode == GameMode.TrainingRoom && playerNum == 2 && UFE.config.trainingModeOptions.p2StunGauge != LifeBarTrainingMode.Normal) return;
        myInfo.currentStunPoints += gaugeGain;
        if (myInfo.currentStunPoints > myInfo.maxStunPoints) myInfo.currentStunPoints = myInfo.maxStunPoints;
    }

    // Remove Stun Gauge
    private void RemoveStunGauge(float gaugeLoss)
    {
        if (isDead || opControlsScript.isDead) return;
        myInfo.currentStunPoints -= gaugeLoss;
        if (myInfo.currentStunPoints < 0) myInfo.currentStunPoints = 0;
    }
    ...
    // Refill Ultra
    private void RefillUltraGauge(){
        AddUltraGauge(myInfo.maxUltraGauge);
    }
	...
	private bool DamageMe(float damage){
		...
		if (opControlsScript != null) {
			finalDamage *= opControlsScript.attackMultiplier;
		}

		finalDamage /= this.defenseMultiplier;

		//change to finalDamage from damahe
		myInfo.currentLifePoints -= finalDamage;
