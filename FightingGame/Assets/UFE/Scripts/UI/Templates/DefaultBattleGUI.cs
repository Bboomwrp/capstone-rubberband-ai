...
public class DefaultBattleGUI : BattleGUI{
	...
	public class PlayerGUI{
        ...
        public Image ultraGaugeMeter;
        public Image stunGaugeMeter;
        public GameObject wonRoundImagesParent;
	...
	public override void DoFixedUpdate (){
		...
		if (this.isRunning){
            ...
            if (UFE.config.gameGUI.hasUltraGauge)
            {
                if (this.player1GUI != null && this.player1GUI.ultraGaugeMeter != null)
                {
                    this.player1GUI.ultraGaugeMeter.fillAmount = UFE.config.player1Character.currentUltraGaugePoints / UFE.config.player1Character.maxUltraGauge;
                }

                if (this.player2GUI != null && this.player2GUI.ultraGaugeMeter != null)
                {
                    this.player2GUI.ultraGaugeMeter.fillAmount = UFE.config.player2Character.currentUltraGaugePoints / UFE.config.player2Character.maxUltraGauge;
                }
            }

            if (UFE.config.gameGUI.hasStunGauge)
            {
                if (this.player1GUI != null && this.player1GUI.stunGaugeMeter != null)
                {
                    this.player1GUI.stunGaugeMeter.fillAmount = UFE.config.player1Character.currentStunPoints / UFE.config.player1Character.maxStunPoints;
                }

                if (this.player2GUI != null && this.player2GUI.stunGaugeMeter != null)
                {
                    this.player2GUI.stunGaugeMeter.fillAmount = UFE.config.player2Character.currentStunPoints / UFE.config.player2Character.maxStunPoints;
                }
            }
	...
	protected override void OnGameBegin (CharacterInfo player1, CharacterInfo player2, StageOptions stage){
		...
		if (this.wonRounds.NotFinishedRounds == null){
			...
			if(
                ...
                player1GUI.wonRoundImagesParent.SetActive(true);
                player1GUI.wonRoundsText.gameObject.SetActive(false);
            }
            else
            {
                if (this.player1GUI != null && this.player1GUI.wonRoundsImages != null && this.player1GUI.wonRoundsImages.Length > 0)
                {
                    for (int i = 0; i < this.player1GUI.wonRoundsImages.Length; ++i)
                    {
                        this.player1GUI.wonRoundsImages[i].enabled = true;
                        this.player1GUI.wonRoundsImages[i].sprite = this.wonRounds.NotFinishedRounds;
                    }
                ...
                {
                    player1GUI.wonRoundImagesParent.SetActive(false);
                    player1GUI.wonRoundsText.text = "";
                    player1GUI.wonRoundsText.gameObject.SetActive(true);
                }
                ...
                Debug.Log(
                    ...
                    "If Won rounds beyond Image's length, it'll show as text."
                    );
...
//				);
            ...
            {
                if (this.player2GUI != null && this.player2GUI.wonRoundsImages != null && this.player2GUI.wonRoundsImages.Length > 0)
                {
                    for (int i = 0; i < this.player2GUI.wonRoundsImages.Length; ++i)
                    {
                        this.player2GUI.wonRoundsImages[i].enabled = true;
                        this.player2GUI.wonRoundsImages[i].sprite = this.wonRounds.NotFinishedRounds;
                    }
                    player2GUI.wonRoundImagesParent.SetActive(true);
                    player2GUI.wonRoundsText.gameObject.SetActive(false);
                }
                else
                {
                    player2GUI.wonRoundImagesParent.SetActive(false);
                    player2GUI.wonRoundsText.text = "";
                    player2GUI.wonRoundsText.gameObject.SetActive(true);
                }
...
//				);
		...
		if (this.timer != null){
			if (UFE.config.roundOptions.hasTimer){
				...
				this.timer.text = UFE.config.roundOptions.timer.ToString().Replace("Infinity", "∞");
			}else{
        ...
        if (UFE.config.gameGUI.hasUltraGauge)
        {
            if (this.player1GUI != null && this.player1GUI.ultraGaugeMeter != null)
            {
                this.player1GUI.ultraGaugeMeter.gameObject.SetActive(true);
                this.player1GUI.ultraGaugeMeter.fillAmount = UFE.config.player1Character.currentUltraGaugePoints / UFE.config.player1Character.maxUltraGauge;
            }

            if (this.player2 != null && this.player2GUI.ultraGaugeMeter != null)
            {
                this.player2GUI.ultraGaugeMeter.gameObject.SetActive(true);
                this.player2GUI.ultraGaugeMeter.fillAmount = UFE.config.player2Character.currentUltraGaugePoints / UFE.config.player2Character.maxUltraGauge;
            }
        }
        else
        {
            if (this.player1GUI != null && this.player1GUI.ultraGaugeMeter != null)
            {
                this.player1GUI.ultraGaugeMeter.gameObject.SetActive(false);
            }

            if (this.player2GUI != null && this.player2GUI.ultraGaugeMeter != null)
            {
                this.player2GUI.ultraGaugeMeter.gameObject.SetActive(false);
            }
        }
        ...
        if (UFE.config.gameGUI.hasStunGauge)
        {
            if (this.player1GUI != null && this.player1GUI.stunGaugeMeter != null)
            {
                this.player1GUI.stunGaugeMeter.gameObject.SetActive(true);
                this.player1GUI.stunGaugeMeter.fillAmount = UFE.config.player1Character.currentStunPoints / UFE.config.player1Character.maxStunPoints;
            }

            if (this.player2 != null && this.player2GUI.stunGaugeMeter != null)
            {
                this.player2GUI.stunGaugeMeter.gameObject.SetActive(true);
                this.player2GUI.stunGaugeMeter.fillAmount = UFE.config.player2Character.currentStunPoints / UFE.config.player2Character.maxStunPoints;
            }
        }
        else
        {
            if (this.player1GUI != null && this.player1GUI.stunGaugeMeter != null)
            {
                this.player1GUI.stunGaugeMeter.gameObject.SetActive(false);
            }

            if (this.player2GUI != null && this.player2GUI.stunGaugeMeter != null)
            {
                this.player2GUI.stunGaugeMeter.gameObject.SetActive(false);
            }
        }
    }
	...
	protected override void OnRoundEnd (CharacterInfo winner, CharacterInfo loser){
        ...
        Debug.Log("Total round : " + (UFE.GetControlsScript(1).roundsWon + UFE.GetControlsScript(2).roundsWon) + " rounds, Player 1 win : " + UFE.GetControlsScript(1).roundsWon + " Player 2 win : " + UFE.GetControlsScript(2).roundsWon);
		...
		if (this.wonRounds.NotFinishedRounds == null){
			...
			if (this.wonRounds.VisibleImages == DefaultBattleGUI.VisibleImages.AllRounds){
				...
				if(
                    ...
                    winnerGUI.wonRoundsText.gameObject.SetActive(false);
                    winnerGUI.wonRoundsImages[winnerControlsScript.roundsWon - 1].sprite = this.wonRounds.WonRounds;
                    winnerGUI.wonRoundImagesParent.SetActive(true);
                }
                else
                {
                    winnerGUI.wonRoundImagesParent.SetActive(false);
                    winnerGUI.wonRoundsText.text = winnerControlsScript.roundsWon + " wins";
                    winnerGUI.wonRoundsText.gameObject.SetActive(true);
...
//					);
	...
	protected override void OnTimer (float time){
		base.OnTimer (time);
		if (this.timer != null) this.timer.text = Mathf.Round(time).ToString().Replace("Infinity", "∞");
	}
