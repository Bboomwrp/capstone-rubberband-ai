			...
			EditorGUILayout.BeginVertical(rootGroupStyle);{
				...
				if (moveInfo.gaugeToggle){
					EditorGUILayout.BeginVertical(subGroupStyle);{
                        ...
                        moveInfo.isUltraMove = EditorGUILayout.Toggle("Ultra Move", moveInfo.isUltraMove, toggleStyle);
                        moveInfo.useUltraGauge = EditorGUILayout.Toggle("Use ultra gauge", moveInfo.useUltraGauge, toggleStyle);
                        ...
                        EditorGUI.BeginDisabledGroup(moveInfo.stopDrainingGauge);
                        {
                            ...
                            if (moveInfo.startDrainingGauge) {
                                ...
                                moveInfo.gaugeDPS = EditorGUILayout.FloatField("- Drain Per Second (%):", moveInfo.gaugeDPS);
                                moveInfo.totalDrain = EditorGUILayout.FloatField("- Total Drain (%):", moveInfo.totalDrain);
				...
				if (animationOptions){
					EditorGUILayout.BeginVertical(subGroupStyle);{
						...
						if (moveInfo.animationClip != null){
                            ...
                            moveInfo.animationSpeed = EditorGUILayout.FloatField("Base Speed:", moveInfo.animationSpeed);

                            float animTime = 0;

                            if (moveInfo.fixedSpeed) {
                                totalFramesTemp = totalFramesOriginal;
                                animTime = moveInfo.animationClip.length * moveInfo.animationSpeed;
                                    ...
                                    EditorGUILayout.BeginVertical(subGroupStyle);
                                    {
                                        ...
                                        for (int i = 0; i < moveInfo.animSpeedKeyFrame.Length; i++) {
                                            ...
                                            EditorGUILayout.BeginVertical(arrayElementStyle);
                                            {
                                                ...
                                                moveInfo.animSpeedKeyFrame[i].speed = EditorGUILayout.FloatField("New Speed:", Mathf.Max(moveInfo.animSpeedKeyFrame[i].speed, 0.2f));
									...
									EditorGUILayout.BeginVertical(arrayElementStyle);{
                                        ...
                                        if (moveInfo.hits[i].hitConditionsToggle) {
                                            EditorGUILayout.BeginVertical(subGroupStyle);
                                            {
                                                ...
                                                moveInfo.hits[i].cancelProjectile = EditorGUILayout.Toggle("Cancel projectile", moveInfo.hits[i].cancelProjectile, toggleStyle);
                                        ...
                                        if (moveInfo.hits[i].hitConfirmType == HitConfirmType.Hit) {
											...
											if (moveInfo.hits[i].continuousHit){
                                                ...
                                                if (moveInfo.hits[i].spaceBetweenHits == Sizes.Custom)
                                                    moveInfo.hits[i].spaceBetweenHitsVal = EditorGUILayout.FloatField("- Seconds between hits:", moveInfo.hits[i].spaceBetweenHitsVal);
                                            }
											...
											if (moveInfo.hits[i].damageOptionsToggle){
												EditorGUILayout.BeginVertical(subGroupStyle);{
                                                    ...
                                                    moveInfo.hits[i].stundamage = EditorGUILayout.FloatField("Stun Damage on Hit:", moveInfo.hits[i].stundamage);
											...
											if (moveInfo.hits[i].forceOptionsToggle){
												EditorGUILayout.BeginVertical(subGroupStyle);{
                                                    ...
                                                    if (moveInfo.hits[i].opponentForceToggle) {
														...
														EditorGUI.BeginDisabledGroup(moveInfo.hits[i].hitType == HitType.MidKnockdown || moveInfo.hits[i].hitType == HitType.HighKnockdown || moveInfo.hits[i].hitType == HitType.Sweep);{
                                                            ...
                                                            if (moveInfo.hits[i].applyDifferentAirForce)
                                                                moveInfo.hits[i].pushForceAir = EditorGUILayout.Vector2Field("Applied Force (Air)", moveInfo.hits[i].pushForceAir);
                                                    ...
                                                    if (moveInfo.hits[i].selfForceToggle) {
                                                        ...
                                                        moveInfo.hits[i].applyDifferentAirForce = EditorGUILayout.Toggle("Apply Different Air Force", moveInfo.hits[i].applyDifferentAirForce, toggleStyle);
                                                        if (moveInfo.hits[i].applyDifferentAirForce)
                                                            moveInfo.hits[i].appliedForceAir = EditorGUILayout.Vector2Field("Applied Force (Air)", moveInfo.hits[i].appliedForceAir);
                                                    ...
                                                    if (moveInfo.hits[i].groundBounce) {
                                                        ...
                                                        if (moveInfo.hits[i].groundBounceToggle) {
                                                            ...
                                                            EditorGUI.BeginDisabledGroup(!moveInfo.hits[i].overrideForcesOnGroundBounce); {
                                                                ...
                                                                moveInfo.hits[i].groundBouncePushForce = EditorGUILayout.Vector2Field("Applied Force", moveInfo.hits[i].groundBouncePushForce);
                                                            } EditorGUI.EndDisabledGroup();
                                                            EditorGUI.indentLevel -= 1;
                                                        }
                                                    }
                                                    ...
                                                    if (moveInfo.hits[i].wallBounce) {
                                                        ...
                                                        if (moveInfo.hits[i].wallBounceToggle) {
                                                            ...
                                                            EditorGUI.BeginDisabledGroup(!moveInfo.hits[i].overrideForcesOnWallBounce); {
                                                                ...
                                                                moveInfo.hits[i].wallBouncePushForce = EditorGUILayout.Vector2Field("Applied Force", moveInfo.hits[i].wallBouncePushForce);
                                                            } EditorGUI.EndDisabledGroup();
                                                            EditorGUI.indentLevel -= 1;
                                                        }
                                                    };
                                            ...
                                            if (moveInfo.hits[i].overrideEventsToggle) {
                                                EditorGUILayout.BeginVertical(subGroupStyle);
                                                {
                                                    ...
                                                    if (moveInfo.hits[i].overrideHitAnimation) moveInfo.hits[i].overrideOnlyOnAirHit = EditorGUILayout.Toggle("- Air Hit Only", moveInfo.hits[i].overrideOnlyOnAirHit, toggleStyle);
                                                    EditorGUIUtility.labelWidth = 240;
                                                    ...
                                                    if (moveInfo.hits[i].overrideHitAnimationBlend) 
                                                        moveInfo.hits[i].newHitBlendingIn = EditorGUILayout.FloatField("- Blending In:", moveInfo.hits[i].newHitBlendingIn);
                                                    EditorGUIUtility.labelWidth = 240;
                                                    ...
                                                    if (moveInfo.hits[i].overrideJuggleWeight) 
                                                        moveInfo.hits[i].newJuggleWeight = EditorGUILayout.FloatField("- New Weight:", moveInfo.hits[i].newJuggleWeight);
                                                    EditorGUIUtility.labelWidth = 240;
                                                    ...
                                                    if (moveInfo.hits[i].overrideCameraSpeed) {
                                                        moveInfo.hits[i].newMovementSpeed = EditorGUILayout.FloatField("- New Movement Speed:", moveInfo.hits[i].newMovementSpeed);
                                                        moveInfo.hits[i].newRotationSpeed = EditorGUILayout.FloatField("- New Rotation Speed:", moveInfo.hits[i].newRotationSpeed);
                                                        moveInfo.hits[i].cameraSpeedDuration = EditorGUILayout.FloatField("- Override Duration:", moveInfo.hits[i].cameraSpeedDuration);
                                                    };
					...
					EditorGUILayout.BeginVertical(subGroupStyle);{
						...
						for (int i = 0; i < moveInfo.opponentOverride.Length; i ++){
							...
							EditorGUILayout.BeginVertical(arrayElementStyle);{
                                ...
                                moveInfo.opponentOverride[i].forceGround = EditorGUILayout.Toggle("Force ground", moveInfo.opponentOverride[i].forceGround, toggleStyle);
					...
					EditorGUILayout.BeginVertical(subGroupStyle);{
						...
						for (int i = 0; i < moveInfo.cameraMovements.Length; i ++){
							...
							EditorGUILayout.BeginVertical(arrayElementStyle);{
								...
								moveInfo.cameraMovements[i].myAnimationSpeed = EditorGUILayout.Slider("Character Animation Speed (%):", moveInfo.cameraMovements[i].myAnimationSpeed, 0, 999);
								moveInfo.cameraMovements[i].opAnimationSpeed = EditorGUILayout.Slider("Opponent Animation Speed (%):", moveInfo.cameraMovements[i].opAnimationSpeed, 0, 999);
								EditorGUIUtility.labelWidth = 150;
					...
					EditorGUILayout.BeginVertical(subGroupStyle);{
						...
						for (int i = 0; i < moveInfo.bodyPartVisibilityChanges.Length; i ++){
							...
							EditorGUILayout.BeginVertical(arrayElementStyle);{
                                ...
                                moveInfo.bodyPartVisibilityChanges[i].left = EditorGUILayout.Toggle("Trigger Left:", moveInfo.bodyPartVisibilityChanges[i].left);
                                moveInfo.bodyPartVisibilityChanges[i].right = EditorGUILayout.Toggle("Trigger Right:", moveInfo.bodyPartVisibilityChanges[i].right);
                                moveInfo.bodyPartVisibilityChanges[i].saveAsDefault = EditorGUILayout.Toggle("Save as Default:", moveInfo.bodyPartVisibilityChanges[i].saveAsDefault);
						...
						for (int i = 0; i < moveInfo.projectiles.Length; i ++){
							...
							EditorGUILayout.BeginVertical(arrayElementStyle);{
								...
								EditorGUI.BeginDisabledGroup(moveInfo.projectiles[i].projectilePrefab == null);{
									...
									if (moveInfo.projectiles[i].totalHits > 1) {
                                        ...
                                        if (moveInfo.projectiles[i].spaceBetweenHits == Sizes.Custom)
                                            moveInfo.projectiles[i].spaceBetweenHitsVal = EditorGUILayout.FloatField("- Seconds between hits:", moveInfo.projectiles[i].spaceBetweenHitsVal);
                                    }

                                    moveInfo.projectiles[i].pushForce = EditorGUILayout.Vector2Field("Push Force:", moveInfo.projectiles[i].pushForce);
                                    moveInfo.projectiles[i].pushForceLast = EditorGUILayout.Vector2Field("Push Force Last:", moveInfo.projectiles[i].pushForceLast);
									...
									if (moveInfo.projectiles[i].damageOptionsToggle){
										EditorGUILayout.BeginVertical(subGroupStyle);{
                                            ...
                                            moveInfo.projectiles[i].damageOnHitFirst = EditorGUILayout.FloatField("Damage on First Hit:", moveInfo.projectiles[i].damageOnHitFirst);
                                            moveInfo.projectiles[i].damageOnBlockFirst = EditorGUILayout.FloatField("Damage on First Block:", moveInfo.projectiles[i].damageOnBlockFirst);
                                            moveInfo.projectiles[i].stunDamageFirst = EditorGUILayout.FloatField("Stun on First Hit:", moveInfo.projectiles[i].stunDamageFirst);
                                            ...
                                            moveInfo.projectiles[i].stunDamage = EditorGUILayout.FloatField("Stun on Hit:", moveInfo.projectiles[i].stunDamage);
                                            moveInfo.projectiles[i].damageOnHitLast = EditorGUILayout.FloatField("Damage on Last Hit:", moveInfo.projectiles[i].damageOnHitLast);
                                            moveInfo.projectiles[i].damageOnBlockLast = EditorGUILayout.FloatField("Damage on Last Block:", moveInfo.projectiles[i].damageOnBlockLast);
                                            moveInfo.projectiles[i].stunDamageLast = EditorGUILayout.FloatField("Stun on Last Hit:", moveInfo.projectiles[i].stunDamageLast);
				...
				EditorGUILayout.BeginVertical(subGroupStyle);{
					...
					for (int i = 0; i < playerConditions.possibleMoveStates.Length; i ++){
						...
						EditorGUILayout.BeginVertical(subArrayElementStyle);{
                        	...
                        	playerConditions.possibleMoveStates[i].isCornor = EditorGUILayout.Toggle("Cornor Move", playerConditions.possibleMoveStates[i].isCornor, toggleStyle);
